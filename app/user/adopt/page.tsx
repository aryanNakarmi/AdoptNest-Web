...
export default function AdoptPage() {
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 border-b border-gray-100">
        ...
        {activeFiltersCount > 0 && (
          <button onClick={clearFilters} className="text-xs text-red-600 hover:text-red-700 font-medium transition">
            Clear all
          </button>
        )}
      </div>
          ...
          <div className="space-y-2">
            {GENDER_OPTIONS.map((g) => (
              <label key={g} className="flex items-center gap-2 cursor-pointer group">
                <input type="radio" name="gender" value={g} checked={genderFilter === g} onChange={() => setGenderFilter(g)} className="accent-red-600 w-3.5 h-3.5" />
                <span className={`text-sm transition ${genderFilter === g ? 'text-red-600 font-semibold' : 'text-gray-700'}`}>{g}</span>
              </label>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {(['All', 'Available', 'Adopted'] as const).map((s) => (
              <label key={s} className="flex items-center gap-2 cursor-pointer group">
                <input type="radio" name="status" value={s} checked={statusFilter === s} onChange={() => setStatusFilter(s)} className="accent-red-600 w-3.5 h-3.5" />
                <span className={`text-sm transition ${statusFilter === s ? 'text-red-600 font-semibold' : 'text-gray-700'}`}>{s}</span>
              </label>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div>
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Adopt a Pet</h1>
      ...
      <div className="relative w-full">
        ...
        <input
          ...
          className="w-full pl-11 pr-10 py-3 border border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 text-gray-900 placeholder-gray-400 bg-white text-sm"
        />
        {searchQuery && (
      ...
      <div className="flex flex-wrap gap-3 bg-white p-3 rounded-lg border border-gray-200">
        <button
          ...
          className={`px-4 py-1.5 rounded-lg font-medium text-sm transition ${activeTab === 'all' ? 'bg-red-600 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
        >
          All Animals ({allPosts.length})
        </button>
        <button
          onClick={() => { setActiveTab('my-adoptions'); clearFilters(); }}
          className={`px-4 py-1.5 rounded-lg font-medium text-sm transition ${activeTab === 'my-adoptions' ? 'bg-red-600 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
        >
        ...
        <button
          ...
          className="ml-auto md:hidden flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-300 text-gray-700 text-sm font-medium hover:bg-gray-50 transition"
        >
          <HiFilter size={14} />
          Filters
          {activeFiltersCount > 0 && (
        ...
        <div className="flex-1 min-w-0 space-y-4">
          {!loading && (
            <div className="flex items-center justify-between flex-wrap gap-2">
              <p className="text-sm text-gray-500">
                ...
                <span className="font-semibold text-gray-900">{activeTab === 'all' ? allPosts.length : myAdoptionsPosts.length}</span> result(s)
              </p>
              <div className="flex flex-wrap gap-2">
                {speciesFilter !== 'All' && (
                  <span className="flex items-center gap-1 bg-red-50 text-red-600 border border-red-200 text-xs px-2.5 py-1 rounded-full font-medium">
                    {speciesFilter}<button onClick={() => setSpeciesFilter('All')}><HiX size={11} /></button>
                  </span>
                )}
                {genderFilter !== 'All' && (
                  <span className="flex items-center gap-1 bg-red-50 text-red-600 border border-red-200 text-xs px-2.5 py-1 rounded-full font-medium">
                    {genderFilter}<button onClick={() => setGenderFilter('All')}><HiX size={11} /></button>
                  </span>
                )}
                {statusFilter !== 'All' && (
                  <span className="flex items-center gap-1 bg-red-50 text-red-600 border border-red-200 text-xs px-2.5 py-1 rounded-full font-medium">
                    {statusFilter}<button onClick={() => setStatusFilter('All')}><HiX size={11} /></button>
                  </span>
                )}
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 bg-white rounded-lg border border-gray-200">
              ...
              {activeFiltersCount > 0 && (
                <button onClick={clearFilters} className="inline-flex items-center gap-2 bg-red-600 text-white px-5 py-2.5 rounded-lg hover:bg-red-700 transition font-medium">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
              {filteredPosts.map((post) => (
                <div key={post._id} className="rounded-xl shadow-md border border-gray-200 overflow-hidden transition transform hover:scale-[1.02] hover:shadow-xl bg-white">
                  ...
                  <div className="p-4 space-y-2">
                    ...
                    <Link href={`/user/adopt/${post._id}`} className="w-full flex items-center justify-center gap-2 px-3 py-1.5 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition text-sm font-medium">
